...
class _CourseViewerDialogState extends State<CourseViewerDialog> {
  ...
  Future<void> _fetchCourseDetails() async {
    ...
    try {
      ...
      if (response.data['success'] == true) {
        final modulesList = response.data['modules'] as List? ?? [];
        ...
        throw StateError('Course details were not returned by the server.');
      }
    } catch (e) {
      if (mounted) {
        setState(() {
          _error = 'Unable to load this course. Please try again.';
          _isLoading = false;
        });
      }
  }
  ...
  Future<void> _markChapterComplete(int chapterId) async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Progress could not be saved. Please try again.')),
        );
        setState(() {
          _isSavingProgress = false;
        });
