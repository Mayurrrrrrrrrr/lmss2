...
class _TrainerCourseBuilderScreenState extends State<TrainerCourseBuilderScreen> {
  ...
  Future<void> _chapterDialog(int moduleId,[Map<String,dynamic>? existing]) async {
    ...
    var uploading=false;
    final data=await showDialog<Map<String,dynamic>>(context:context,builder:(x)=>StatefulBuilder(builder:(context,setLocal)=>AlertDialog(title:Text(existing==null?'Add chapter':'Edit chapter'),content:SizedBox(width:560,child:Column(mainAxisSize:MainAxisSize.min,children:[TextField(controller:title,decoration:const InputDecoration(labelText:'Chapter title')),DropdownButtonFormField<String>(initialValue:type,items:const ['html','youtube','pdf','word','ppt','video','audio','image','txt'].map((v)=>DropdownMenuItem(value:v,child:Text(v.toUpperCase()))).toList(),onChanged:(v)=>setLocal(()=>type=v!),decoration:const InputDecoration(labelText:'Content type')),TextField(controller:content,maxLines:type=='html'?6:2,decoration:InputDecoration(labelText:type=='html'?'HTML content':'Content URL or uploaded file')),if(type!='html'&&type!='youtube')Padding(padding:const EdgeInsets.only(top:12),child:Row(children:[OutlinedButton.icon(onPressed:uploading?null:()async{final picked=await FilePicker.platform.pickFiles(withData:true);if(picked==null)return;final file=picked.files.single;if(file.bytes==null)return;setLocal(()=>uploading=true);try{final uploaded=await _api.uploadTrainerContent(file.bytes!,file.name);content.text=uploaded['content_path']?.toString()??'';}finally{setLocal(()=>uploading=false);}},icon:uploading?const SizedBox(width:16,height:16,child:CircularProgressIndicator(strokeWidth:2)):const Icon(Icons.upload_file),label:Text(uploading?'Uploading...':'Upload file')),const SizedBox(width:12),const Expanded(child:Text('PDF, PPT/PPTX, Word, video, audio, images or text; maximum 50 MB.'))]))])),actions:[TextButton(onPressed:uploading?null:()=>Navigator.pop(x),child:const Text('Cancel')),FilledButton(onPressed:uploading?null:()=>Navigator.pop(x,{'title':title.text.trim(),'content_type':type,'content_path':content.text.trim(),'sequence_order':(existing?['sequence_order'] as num?)?.toInt()??100,'duration_seconds':(existing?['duration_seconds'] as num?)?.toInt()??60}),child:const Text('Save'))])));
