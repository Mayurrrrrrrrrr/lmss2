...
class _TrainerQuestionsScreenState extends State<TrainerQuestionsScreen> {
  ...
  Future<void> _importCsv()async{try{final picked=await FilePicker.pickFiles(type:FileType.custom,allowedExtensions:const['csv'],withData:true);if(picked==null)return;final bytes=picked.files.single.bytes;if(bytes==null)throw Exception('Could not read the selected file');final result=await _api.importQuizQuestions(widget.quizId,bytes);_reload();if(mounted)ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text(result['message'].toString())));}catch(e){if(mounted)ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text('Import failed: $e')));}}
  Future<void> _exportCsv()async{try{final bytes=await _api.exportQuizQuestions(widget.quizId);await FilePicker.saveFile(dialogTitle:'Save question CSV',fileName:'quiz_${widget.quizId}_questions.csv',type:FileType.custom,allowedExtensions:const['csv'],bytes:Uint8List.fromList(bytes));}catch(e){if(mounted)ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text('Export failed: $e')));}}
  @override Widget build(BuildContext context)=>LmsShell(title:widget.title.isEmpty?'Quiz questions':widget.title,actions:[IconButton(tooltip:'Import CSV',onPressed:_importCsv,icon:const Icon(Icons.upload_file)),IconButton(tooltip:'Export CSV',onPressed:_exportCsv,icon:const Icon(Icons.download)),IconButton(tooltip:'Refresh questions',onPressed:_reload,icon:const Icon(Icons.refresh))],floatingActionButton:FloatingActionButton.extended(onPressed:()=>_save(),icon:const Icon(Icons.add),label:const Text('Add question')),body:FutureBuilder<List<Map<String,dynamic>>>(future:_questions,builder:(context,snapshot){if(snapshot.connectionState!=ConnectionState.done)return const LmsLoadingState(label:'Loading questions');if(snapshot.hasError)return LmsErrorState(message:'We could not load quiz questions.',onRetry:_reload);final questions=snapshot.data??const[];if(questions.isEmpty)return LmsEmptyState(icon:Icons.quiz_outlined,title:'No questions yet',message:'Import a CSV or add the first question manually.',action:FilledButton.icon(onPressed:()=>_save(),icon:const Icon(Icons.add),label:const Text('Add question')));return ListView.builder(padding:const EdgeInsets.all(20),itemCount:questions.length,itemBuilder:(context,index){final q=questions[index];final options=List<Map<String,dynamic>>.from(q['options']??const[]);return Card(child:ExpansionTile(leading:CircleAvatar(child:Text('${index+1}')),title:Text(q['text']?.toString()??''),subtitle:Text(q['difficulty']?.toString()??''),trailing:PopupMenuButton<String>(onSelected:(action)async{if(action=='edit')await _save(q);if(action=='delete'){await _api.deleteTrainerQuestion(q['id'] as int);_reload();}},itemBuilder:(_)=>const[PopupMenuItem(value:'edit',child:Text('Edit')),PopupMenuItem(value:'delete',child:Text('Delete'))]),children:options.map((o)=>ListTile(leading:Icon(o['is_correct']==true?Icons.check_circle:Icons.circle_outlined,color:o['is_correct']==true?Colors.green:null),title:Text(o['text']?.toString()??''))).toList()));});}));
}
