...
class TrainerRoleplaysScreen extends StatefulWidget {
  ...
  @override
  State<TrainerRoleplaysScreen> createState() => _TrainerRoleplaysScreenState();
}
...
class _TrainerRoleplaysScreenState extends State<TrainerRoleplaysScreen> {
              ...
              child: Column(
                ...
                children: [
                  ...
                  Autocomplete<String>(
                    ...
                    onSelected: (value) => topic.text = value,
                    fieldViewBuilder: (context, controller, focus, submit) {
                      controller.addListener(
                        () => topic.text = controller.text,
                      );
                      return TextField(
                        controller: controller,
                        focusNode: focus,
                        decoration: const InputDecoration(
                          labelText: 'Scenario topic',
                        ),
                      );
                    },
                  ),
                  ...
                  SegmentedButton<String>(
                    segments: const [
                      ButtonSegment(value: 'individual', label: Text('People')),
                      ButtonSegment(value: 'store', label: Text('Stores')),
                      ButtonSegment(value: 'manager', label: Text('Managers')),
                    ],
                    selected: {mode},
                    onSelectionChanged: (value) =>
                        update(() => mode = value.first),
                  ),
                        ...
                        if (mode == 'individual')
                          ..._participants.map((person) {
                            ...
                            return CheckboxListTile(
                              dense: true,
                              value: users.contains(id),
                              title: Text(
                                person['full_name']?.toString() ??
                                    person['username']?.toString() ??
                                    '',
                              ),
                              subtitle: person['store_code'] == null
                                  ? null
                                  : Text(person['store_code'].toString()),
                              onChanged: (v) => update(
                                () => v == true
                                    ? users.add(id)
                                    : users.remove(id),
            ...
            FilledButton(
              onPressed: () async {
                ...
                await _api.assignTrainerRoleplays(
                  ...
                  day: day.text.trim(),
                  scenarioTopic: topic.text.trim(),
                  userIds: mode == 'individual' ? users.toList() : const [],
                  storeCodes: mode == 'store' ? stores.toList() : const [],
                  managerNames: mode == 'manager'
                      ? managers.toList()
                      : const [],
                );
  ...
  Future<void> _evaluate(Map<String, dynamic> session) async {
    ...
    final saved = await showDialog<bool>(
      ...
      builder: (dialogContext) => StatefulBuilder(
        builder: (context, update) => AlertDialog(
          title: const Text('Evaluate roleplay'),
          content: SizedBox(
            width: 480,
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                Text('${session['full_name']} - ${session['scenario_topic']}'),
                Slider(
                  value: score,
                  min: 1,
                  max: 5,
                ...
                TextField(
                  controller: notes,
                  maxLines: 4,
                  decoration: const InputDecoration(labelText: 'Debrief notes'),
                ),
              ],
            ),
          ),
          actions: [
            ...
            FilledButton(
              onPressed: () async {
                await _api.evaluateTrainerRoleplay(
                  session['id'] as int,
                  score,
                  notes.text.trim(),
                );
  ...
  Widget build(BuildContext context) => LmsShell(
    ...
    floatingActionButton: FloatingActionButton.extended(
      ...
      label: const Text('Assign roleplay'),
    ),
    body: _loading
        ? const Center(child: CircularProgressIndicator())
        : _error != null
        ? Center(child: Text('Could not load roleplays: $_error'))
        : ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return Card(
                child: ListTile(
                  ...
                  subtitle: Text(
                    '${item['full_name'] ?? item['username']} - ${item['week_no']} / ${item['day']}\n$status${item['observer_score'] != null ? ' - Score ${item['observer_score']}/5' : ''}',
                  ),
                  isThreeLine: true,
