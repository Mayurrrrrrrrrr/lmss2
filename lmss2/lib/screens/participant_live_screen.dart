...
class _ParticipantLiveJoinScreenState extends State<ParticipantLiveJoinScreen> {
  ...
  @override Widget build(BuildContext context) => LmsShell(title: 'Join Live Quiz', rootPage: true, body: Center(child: Card(child: Padding(padding: const EdgeInsets.all(28), child: SizedBox(width: 420, child: Column(mainAxisSize: MainAxisSize.min, children: [const Icon(Icons.wifi_tethering, size: 64), const SizedBox(height: 16), Text('Enter the access code shown by your trainer', style: Theme.of(context).textTheme.titleLarge, textAlign: TextAlign.center), const SizedBox(height: 20), TextField(controller: code, textCapitalization: TextCapitalization.characters, maxLength: 12, onSubmitted: (_) => join(), decoration: const InputDecoration(labelText: 'Access code')), if (error != null) Padding(padding: const EdgeInsets.only(top: 8), child: Text('We could not join that session. Check the code and try again.', style: TextStyle(color: Theme.of(context).colorScheme.error))), const SizedBox(height: 12), FilledButton.icon(onPressed: busy ? null : join, icon: busy ? const SizedBox.square(dimension: 18, child: CircularProgressIndicator(strokeWidth: 2)) : const Icon(Icons.login), label: const Text('Join session'))]))))));
}
