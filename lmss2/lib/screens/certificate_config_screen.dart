...
class _CertificateConfigScreenState extends State<CertificateConfigScreen>{
 ...
 @override Widget build(BuildContext context)=>LmsShell(title:'Certificate — ${widget.courseTitle}',actions:[TextButton(onPressed:()async{await api.resetCertificateConfig(widget.courseId);await load();},child:const Text('Reset defaults'))],body:loading?const Center(child:CircularProgressIndicator()):error!=null?const Center(child:Text('Certificate design is temporarily unavailable.')):ListView(padding:const EdgeInsets.all(24),children:[const Text('Certificate content',style:TextStyle(fontSize:22,fontWeight:FontWeight.bold)),const SizedBox(height:12),...textKeys.map((k)=>Padding(padding:const EdgeInsets.only(bottom:12),child:TextField(controller:controls[k],maxLines:k=='body_text'?3:1,decoration:InputDecoration(labelText:label(k))))),const SizedBox(height:12),const Text('Layout offsets',style:TextStyle(fontSize:20,fontWeight:FontWeight.bold)),const SizedBox(height:12),Wrap(spacing:12,runSpacing:12,children:numberKeys.map((k)=>SizedBox(width:210,child:TextField(controller:controls[k],keyboardType:TextInputType.number,decoration:InputDecoration(labelText:label(k))))).toList()),const SizedBox(height:24),Align(alignment:Alignment.centerRight,child:FilledButton.icon(onPressed:saving?null:save,icon:const Icon(Icons.save),label:Text(saving?'Saving…':'Save certificate design')))]));
}
