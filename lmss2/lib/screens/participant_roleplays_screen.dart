...
class _ParticipantRoleplaysScreenState extends State<ParticipantRoleplaysScreen> with SingleTickerProviderStateMixin {
  ...
  Widget _list(List<Map<String,dynamic>> items,String state)=>items.isEmpty?LmsEmptyState(icon:Icons.video_camera_front_outlined,title:'No $state roleplays',message:'Roleplay activities in this stage will appear here.'):ListView.builder(padding:const EdgeInsets.all(20),itemCount:items.length,itemBuilder:(context,index){final item=items[index];return Card(child:ListTile(leading:const CircleAvatar(child:Icon(Icons.video_camera_front)),title:Text(item['scenario_topic']?.toString()??''),subtitle:Text('${item['week_no']} / ${item['day']}${item['observer_score']!=null?'\nScore: ${item['observer_score']}/5':''}${item['debrief_notes']!=null?'\n${item['debrief_notes']}':''}'),isThreeLine:item['observer_score']!=null,trailing:state=='assigned'?FilledButton(onPressed:()=>_submit(item),child:const Text('Submit')):null));});
  @override Widget build(BuildContext context)=>LmsShell(title:'My Roleplays',rootPage:true,actions:[IconButton(tooltip:'Refresh roleplays',onPressed:_reload,icon:const Icon(Icons.refresh))],body:Column(children:[TabBar(controller:_tabs,tabs:const[Tab(text:'Assigned'),Tab(text:'Pending'),Tab(text:'Completed')]),Expanded(child:FutureBuilder<Map<String,dynamic>>(future:_data,builder:(context,snapshot){if(snapshot.connectionState!=ConnectionState.done)return const LmsLoadingState(label:'Loading roleplays');if(snapshot.hasError)return LmsErrorState(message:'We could not load your roleplays.',onRetry:_reload);final data=snapshot.data??{};List<Map<String,dynamic>> rows(String key)=>List<Map<String,dynamic>>.from(data[key]??const[]);return TabBarView(controller:_tabs,children:[_list(rows('assigned'),'assigned'),_list(rows('pending'),'pending'),_list(rows('completed'),'completed')]);}))]));
}
